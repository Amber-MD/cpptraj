...
Analysis::RetType Analysis_Rms2d::Setup(ArgList& analyzeArgs, DataSetList* DSL, DataFileList* DFL, int debugIn)
{
  ...
  DataFile* rmsdFile = DFL->AddDataFile(outfilename, analyzeArgs);
  std::string reftrajname = analyzeArgs.GetStringKey("reftraj");
  if (!reftrajname.empty()) {
    RefParm_ = DSL->GetTopology(analyzeArgs); // TODO Use coords set
    useReferenceTraj_ = true;
  } else
    useReferenceTraj_ = false;
  // Check for correlation. 
  DataFile* corrfile = DFL->AddDataFile(analyzeArgs.GetStringKey("corr"), analyzeArgs);
  ...
  if (useReferenceTraj_) {
    ...
    RefTraj_ = (DataSet_Coords*)DSL->FindCoordsSet( reftrajname );
    if (RefTraj_ == 0) {
      ...
      DataSet_Coords_TRJ* DCT = (DataSet_Coords_TRJ*)
                                DSL->AddSet(DataSet::TRAJ, reftrajname, "RmsRefTraj");
  ...
  rmsdataset_ = (DataSet_MatrixFlt*)DSL->AddSet( DataSet::MATRIX_FLT, 
  ...
  if (corrfile != 0) {
    Ct_ = DSL->AddSet( DataSet::DOUBLE, MetaData(rmsdataset_->Meta().Name(), "Corr") );
