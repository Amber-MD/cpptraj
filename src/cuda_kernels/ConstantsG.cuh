 ...
 * All constants stay double, as otherwise precision might be lost. However, on the GPU a nice
 * balance between calculations in double and single precision are important (weighing mostly on
 * single precision). Therfore, to achieve maximum parallelization, cast to float if needed.
 */

#ifndef CONSTANTS_CUH
#define CONSTANTS_CUH

namespace ConstantsG {
  const double PI           = 3.141592653589793;
  const double TWOPI        = 6.283185307179586;  
  const double FOURPI       = 12.566370614359172;
  const double FOURTHIRDSPI = 4.1887902047863909; 
  const double FOURFIFTHSPI = 2.5132741228718345;
  const double PIOVER2      = 1.5707963267948966; 
  /// Convert degrees to radians
  const double DEGRAD       = 0.017453292519943295; 
  /// Convert radians to degrees
  const double RADDEG       =   57.29577951308232;
  /// For checking floating point zero
  const double SMALL        = 0.00000000000001;
  const double HUGEG        = 1E30;
