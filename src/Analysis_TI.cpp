...
// Analysis_TI::Setup()
Analysis::RetType Analysis_TI::Setup(ArgList& analyzeArgs, DataSetList* DSL, DataFileList* DFL, int debugIn)
{
  ...
  DataFile* curveout = DFL->AddDataFile(analyzeArgs.GetStringKey("curveout"), analyzeArgs);
  ...
  dAout_ = DSL->AddSet(DataSet::XYMESH, setname, "TI");
  ...
  for (Iarray::const_iterator it = nskip_.begin(); it != nskip_.end(); ++it) {
    md.SetIdx( *it );
    DataSet* ds = DSL->AddSet(DataSet::XYMESH, md);
