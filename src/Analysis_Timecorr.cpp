...
Analysis::RetType Analysis_Timecorr::Setup(ArgList& analyzeArgs, DataSetList* DSL, DataFileList* DFL, int debugIn)
{
  // Get Vectors
  ...
  vinfo1_ = (DataSet_Vector*)DSL->FindSetOfType( vec1name, DataSet::VECTOR );
  if (vinfo1_==0) {
  ...
  if (!vec2name.empty()) {
    vinfo2_ = (DataSet_Vector*)DSL->FindSetOfType( vec2name, DataSet::VECTOR );
    if (vinfo2_==0) {
  ...
  if (setname.empty())
    setname = DSL->GenerateDefaultName("TC");
  ...
  DataFile* dataout = 0;
  if (!ptrajformat_) {
    dataout = DFL->AddDataFile( filename, analyzeArgs );
    if (dplr_) {
      ...
      outfile_ = DFL->AddCpptrajFile( dplrname, "Timecorr dipolar", DataFileList::TEXT, true );
      if (outfile_ == 0) return Analysis::ERR;
    }
  } else {
    outfile_ = DFL->AddCpptrajFile( filename, "Timecorr output" );
    if (outfile_ == 0) return Analysis::ERR;
  }
  // Set up output DataSets
  tc_p_ = DSL->AddSet( DataSet::DOUBLE, MetaData(setname, "P"));
  if (tc_p_ == 0) return Analysis::ERR;
  tc_p_->SetLegend( Plegend_[order_] );
  if (dataout != 0) dataout->AddDataSet( tc_p_ );
  if (dplr_) {
    tc_c_ = DSL->AddSet( DataSet::DOUBLE, MetaData(setname, "C"));
    tc_r3r3_ = DSL->AddSet( DataSet::DOUBLE, MetaData(setname, "R3R3"));
