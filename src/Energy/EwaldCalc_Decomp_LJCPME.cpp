...
EwaldCalc_Decomp_LJCPME::EwaldCalc_Decomp_LJCPME() :
  Recip_(PME_Recip::COULOMB)
{}

/** Set up PME parameters. */
int EwaldCalc_Decomp_LJCPME::Init(Box const& boxIn, EwaldOptions const& pmeOpts, int debugIn)
{
  // Sanity check
  if (!pmeOpts.IsPmeType()) {
    mprinterr("Internal Error: EwaldCalc_Decomp_LJCPME::Init(): Options were not set up for PME.\n");
    return 1;
  }
  mprintf("\tDecomposed Particle Mesh Ewald params:\n");
  if (NBengine_.ModifyEwaldParams().InitEwald(boxIn, pmeOpts, debugIn)) {
    mprinterr("Error: Decomposable PME (with LJC for VDW) calculation init failed.\n");
    return 1;
  }
  VDW_LR_.SetDebug( debugIn );
  if (Recip_.InitRecip(pmeOpts, debugIn)) {
    mprinterr("Error: PME (with LJC for VDW) recip init failed.\n");
    return 1;
  }
...
int EwaldCalc_Decomp_LJCPME::Setup(Topology const& topIn, AtomMask const& maskIn) {
  if (NBengine_.ModifyEwaldParams().SetupEwald(topIn, maskIn)) {
    mprinterr("Error: PME (with LJC for VDW) calculation setup failed.\n");
    return 1;
  }
  if (VDW_LR_.Setup_VDW_Correction( topIn, maskIn )) {
    mprinterr("Error: PME (with LJC for VDW) calculation long range VDW correction setup failed.\n");
    return 1;
  }
...
/** Calculate full decomposed nonbonded energy with PME */
int EwaldCalc_Decomp_LJCPME::CalcNonbondEnergy(Frame const& frameIn, AtomMask const& maskIn,
...
void EwaldCalc_Decomp_LJCPME::Timing(double total) const {
