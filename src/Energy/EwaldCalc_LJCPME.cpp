...
EwaldCalc_LJCPME::EwaldCalc_LJCPME() :
  Recip_(PME_Recip::COULOMB)
{}

/** Set up PME parameters. */
int EwaldCalc_LJCPME::Init(Box const& boxIn, EwaldOptions const& pmeOpts, int debugIn)
{
  // Sanity check
  if (!pmeOpts.IsPmeType()) {
    mprinterr("Internal Error: EwaldCalc_LJCPME::Init(): Options were not set up for PME.\n");
    return 1;
  }
  mprintf("\tParticle Mesh Ewald (with LJC for VDW) params:\n");
  if (NBengine_.ModifyEwaldParams().InitEwald(boxIn, pmeOpts, debugIn)) {
    mprinterr("Error: PME with LJC calculation init failed.\n");
    return 1;
  }
  VDW_LR_.SetDebug( debugIn );
  if (Recip_.InitRecip(pmeOpts, debugIn)) {
    mprinterr("Error: PME with LJC recip init failed.\n");
    return 1;
  }
...
int EwaldCalc_LJCPME::Setup(Topology const& topIn, AtomMask const& maskIn) {
  if (NBengine_.ModifyEwaldParams().SetupEwald(topIn, maskIn)) {
    mprinterr("Error: PME with LJC calculation setup failed.\n");
    return 1;
  }
  if (VDW_LR_.Setup_VDW_Correction( topIn, maskIn )) {
    mprinterr("Error: PME with LJC calculation long range VDW correction setup failed.\n");
    return 1;
  }
  if (!topIn.Nonbond().Has_C_Coeff()) {
    mprinterr("Error: Cannot set up PME with LJC; topology '%s' does not have LJ 12-6-4 (C) terms.\n", topIn.c_str());
    return 1;
  }
...
int EwaldCalc_LJCPME::CalcNonbondEnergy(Frame const& frameIn, AtomMask const& maskIn,
...
void EwaldCalc_LJCPME::Timing(double total) const {
