...
// Analysis_IRED::Setup()
Analysis::RetType Analysis_IRED::Setup(ArgList& analyzeArgs, DataSetList* DSL, DataFileList* DFL, int debugIn)
{
  debug_ = debugIn;
  ...
  DataFile* orderout = DFL->AddDataFile(analyzeArgs.GetStringKey("orderparamfile"), analyzeArgs);
  DataFile* outfile = 0;
  std::string filename = analyzeArgs.GetStringKey("out");
  if (!filename.empty()) {
    outfile  = DFL->AddDataFile(filename);
    cmtfile_ = DFL->AddDataFile(filename + ".cmt");
    cjtfile_ = DFL->AddDataFile(filename + ".cjt");
  }
  DataFile* ds2matfile = DFL->AddDataFile(analyzeArgs.GetStringKey("ds2matrix"));
  // Output data sets
  dsname_ = analyzeArgs.GetStringKey("name");
  if (dsname_.empty()) dsname_ = DSL->GenerateDefaultName("IRED");
  data_s2_ = DSL->AddSet(DataSet::FLOAT, MetaData(dsname_, "S2"));
  ...
  data_plateau_ = DSL->AddSet(DataSet::DOUBLE, MetaData(dsname_, "Plateau"));
  if (data_plateau_ == 0) return Analysis::ERR;
  data_plateau_->SetupFormat().SetFormatWidthPrecision(12,8);
  data_tauM_ = DSL->AddSet(DataSet::DOUBLE, MetaData(dsname_, "TauM"));
  ...
  if (ds2matfile != 0) {
    data_ds2_mat_ = DSL->AddSet(DataSet::MATRIX_DBL, MetaData(dsname_, "dS2"));
  ...
  DataFile* noefile = 0;
  if (relax_) {
    noefile = DFL->AddDataFile(analyzeArgs.GetStringKey("noefile"), analyzeArgs);
    data_t1_  = DSL->AddSet(DataSet::DOUBLE, MetaData(dsname_, "T1"));
    data_t2_  = DSL->AddSet(DataSet::DOUBLE, MetaData(dsname_, "T2"));
    data_noe_ = DSL->AddSet(DataSet::DOUBLE, MetaData(dsname_, "NOE"));
