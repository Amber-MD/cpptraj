...
int ForLoop_mask::SetupFor(CpptrajState& State, std::string const& expr, ArgList& argIn) {
  ...
  if      ( expr == "atoms"       ) SetType( ATOMS );
  else if ( expr == "residues"    ) SetType( RESIDUES );
  else if ( expr == "molecules"   ) SetType( MOLECULES );
  else if ( expr == "molfirstres" ) SetType( MOLFIRSTRES );
  else if ( expr == "mollastres"  ) SetType( MOLLASTRES );
  else {
             ...
             VarType() == MOLLASTRES)
  {
    ...
    for (AtomMask::const_iterator at = currentMask.begin(); at != currentMask.end(); ++at) {
      ...
      if (mol != curMol) {
        if (VarType() == MOLECULES)
          Idxs_.push_back( mol );
        else {
          int res;
          if (VarType() == MOLFIRSTRES)
  ...
  std::string description(std::string(TypeStr[VarType()]) +
