/*
   ...
   Modified by Dan Roe for cpptraj, 2011 November
 */
// ---------- CSTDBLIB includes ------------------------------------------------
#include <stdio.h>
#include <stdlib.h>
...
static void check_convex_edges (ne, convex_edges, circlelist)
	 int ne;
	 EDGE convex_edges[];
	 CIRCLE circlelist[];
{
...
static void draw_circle (narc, npt, n, atomfile, bondfile, propfile,
			 center, rad, zaxis, resnum)
	 int *narc;
	 int *npt;
	 int n;
	 FILE *atomfile;
...
/******************************************************************/
static void dump_cycle (concave_cycle, concave_edge)
	 CONCAVE_CYCLE *concave_cycle;
	 EDGE concave_edge[];
{
  int i, ie;
...
/*****************************************************************/
static int get_cycle_id (cusp_edge, icusp, jcusp, flag)
	 CUSP_EDGE cusp_edge[];
	 int icusp, jcusp, flag;
{
...
/*********************************************************************/
static void make_circle (probe, probe_rad,
			 nc, circle)
	 PROBE probe[];
	 REAL_T probe_rad;
	 int *nc;
...
/***************************************************************/
static void copy_vert (va, vb)
	 EXTREME_VERTEX *va, *vb;
{
...
static void get_xvertex (nspan, four_cusp, concave_edge, cusp_edge, vertex,
			 concave_circle, group, ig, cusp_pair, xvertex)
	 int nspan;					/* number of cusps that span the group */
	 int four_cusp[];
	 EDGE concave_edge[];
	 CUSP_EDGE cusp_edge[];
	 VERTEX vertex[];
...
static void get_2_cycles (cusp_edge, ix, jx, xvertex, cusp_pair, icycle, jcycle)
	 CUSP_EDGE cusp_edge[];
	 int ix, jx;
	 EXTREME_VERTEX xvertex[];
	 CUSP_PAIR cusp_pair[];
...
/***************************************************************/
static void add_2_cusps (xvertex, n_cusps, cusp_edge, concave_cycle,
			 n_concave_edges,
...

{
  ...
  for (i = 0; i < 2; ++i) {
	...
	add_2cusp_verts (nverts, vertex, cusp_pair, xvertex, ix, jx,
...
/* we already know that three_cusp[0] is in pair 0,
  ...
  nspan = 1;
  get_3_xvertex (nspan, three_cusp, concave_edge, cusp_edge, vertex,
				 concave_circle, group, ig, cusp_pair, xvertex);
...
/* should have 4 xvertex[] elements */
  ...
  get_faces (trim_face, &nfaces, three_cusp, 3, cusp_edge,
			 concave_cycle, broken_concave_face);
  ...
  if (case_type == 0) {
	...
	split_3_cusps (three_cusp, 3, cusp_edge, n_cusps, xvertex, 2,
				   cusp_pair, vertex, n_concave_edges,
				   concave_edge, concave_circle);
	...
	for (i = 0; i < nfaces; ++i) {
	  ...
	  split_face (iface, n_broken_concave_faces, broken_concave_face, n_concave_cycles, concave_cycle,
		cusp_edge, cusp_list, n_list, n_concave_edges, concave_edge, vertex,
				  concave_circle);
	}
	return;
  }
  ...
  if (case_type == 1) {
	...
	if (xvertex[0].vert == 1) {
	  add_1_vert (nverts, vertex, cusp_pair[ip].vert1);
	  xvertex[0].vert_index = (*nverts) - 1;
	  add_1_vert (nverts, vertex, cusp_pair[ip].vert2);
	...
	add_2cusp_verts (nverts, vertex, cusp_pair, xvertex, 0, 4,
				  concave_circle, (*n_concave_circles) - 1 );
	...
	cusp_intersect_2 (cusp_edge, middle_cusp, new_cusp_t, probe, iprobe,
					  concave_edge, vertex, concave_circle,
...
static void split_3_cusps (cusp_list, nlist, cusp_edge, n_cusps, xvertex, nx, cusp_pair,
			   vertex, n_concave_edges, concave_edge, concave_circle)
	 int cusp_list[];
	 int nlist;
	 CUSP_EDGE cusp_edge[];
	 ...
	 CIRCLE concave_circle[];
{
  ...
  for (i = 0; i < nlist; ++i) {
	...
	/* now we are ready to split the cusp */
	add_edge (n_concave_edges, concave_edge, concave_edge[iedge].vert1,
			  cut_vert[0], icircle, vertex, concave_circle);
	make_new_cusp (n_cusps, cusp_edge, icusp, (*n_concave_edges) - 1);
	add_edge (n_concave_edges, concave_edge, cut_vert[1],
...
static int get_cusps (four_cusp, group, ig, cusp_pair)
	 int four_cusp[];
	 CUSP_GROUP group[];
	 int ig;
	 CUSP_PAIR cusp_pair[];
...
static void trim_4_cusps (probe, nverts, vertex, n_concave_edges,
			  concave_edge, n_concave_circles, concave_circle,
			  probe_rad, n_broken_concave_faces, broken_concave_face,
			  concave_cycle, n_concave_cycles, cusp_edge, n_cusps,
			  cusp_pair, n_cusp_pairs, group, ig)
	 ...
	 int ig;
{
  ...
  split_old_cusps (four_cusp, 4, cusp_edge, n_cusps, xvertex, 4,
				   cusp_pair, vertex, n_concave_edges,
				   concave_edge, concave_circle);

  cusp_stop = *n_cusps;

  get_faces (trim_face, &nfaces, four_cusp, 4, cusp_edge,
			 concave_cycle, broken_concave_face);
  ...
  for (i = 0; i < nfaces; ++i) {
	...
	split_face (iface, broken_concave_face,
				n_concave_cycles, concave_cycle,
				cusp_edge, cusp_list, n_list,
				n_concave_edges, concave_edge, vertex, concave_circle);
  }
...
static void get_3_xvertex (nspan, three_cusp, concave_edge, cusp_edge, vertex,
			   concave_circle, group, ig, cusp_pair, xvertex)
...
static void add_1_cusp (xvertex, n_cusps, cusp_edge, concave_cycle,
			n_concave_edges,
			concave_edge, nverts, vertex, probe, n_concave_circles,
			concave_circle, probe_rad, cusp_pair)
	 EXTREME_VERTEX xvertex[];
...
static void cusp_intersect_2 (cusp_edge, icusp, jcusp, probe, iprobe,
				  concave_edge, vertex, concave_circle,
				  probe_rad, cusp_pair, n_cusp_pairs, concave_cycle)
	 CUSP_EDGE cusp_edge[];
	 int icusp, jcusp;
...
// ---------- Allocate Memory --------------------
  ...
  if ((concave_circle_list = (CIRCLE *) malloc (
		NUM_CIRCLE * natm_sel * sizeof (CIRCLE))) == NULL) {
	fprintf (stderr, "Unable to allocate space for concave_circle_list\n");
	error_status++;
  }
  ...
  if ((broken_concave_face = (BROKEN_CONCAVE_FACE *) malloc (
	...
	fprintf (stderr, "Unable to allocate space for broken_concave_face\n");
	error_status++;
  }
...
//#endif
  ...
  /* calculate areas                                                       */
    ...
    broken_concave_area (probe_rad, n_broken_concave_faces, broken_concave_face,
