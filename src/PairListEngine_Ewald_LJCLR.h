#ifndef INC_PAIRLISTENGINE_EWALD_LJCLR_H
#define INC_PAIRLISTENGINE_EWALD_LJCLR_H
#include "Energy/Ene_LJ_12_6_4.h"
#include "Energy/EwaldParams.h"
#include "Energy/Kernel_EwaldAdjust.h"
#include "PairList.h"
namespace Cpptraj {
/// Direct space nonbond calculation using pairlist with Ewald and LJ 12-6-4 VDW plus LR correction
template <typename T>
class PairListEngine_Ewald_LJCLR {
  public:
    PairListEngine_Ewald_LJCLR() {}
    // -------------------------------------------
                         ...
                         PairList::AtmType const& atom1)
    {
      ...
      if (nbindex > -1) {
        ...
        double LJC = EW_.GetLJC( nbindex );
        NonbondType const& LJ = EW_.GetLJ( nbindex );
        T e_vdw = Cpptraj::Energy::Ene_LJ_12_6_4<T>(rij2, LJ.A(), LJ.B(), LJC);
        Evdw_ += (e_vdw * vswitch);
      }
    }
...
#   ifdef _OPENMP
    ...
    void operator+=(PairListEngine_Ewald_LJCLR const& rhs) {
      Evdw_ += rhs.Evdw_;
      Eelec_ += rhs.Eelec_;
...
#ifdef _OPENMP
#pragma omp declare reduction( + : PairListEngine_Ewald_LJCLR<double> : omp_out += omp_in ) initializer( omp_priv = omp_orig )
#endif
