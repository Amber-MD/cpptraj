...
Analysis::RetType Analysis_Matrix::Setup(ArgList& analyzeArgs, DataSetList* DSL, DataFileList* DFL, int debugIn)
{
#ifdef NO_MATHLIB
  ...
  matrix_ = (DataSet_2D*)DSL->FindSetOfType( mname, DataSet::MATRIX_DBL );
  if (matrix_ == 0)
    matrix_ = (DataSet_2D*)DSL->FindSetOfType( mname, DataSet::MATRIX_FLT );
  if (matrix_ == 0) {
  ...
  if (nmwizopt_) { 
    ...
    nmwizfile_ = DFL->AddCpptrajFile(analyzeArgs.GetStringKey("nmwizfile"), "NMwiz output",
                                       DataFileList::TEXT, true);
    Topology* parmIn = DSL->GetTopology( analyzeArgs); // TODO: Include with matrix
    if (parmIn == 0) {
  ...
  if (thermopt_) {
    outthermo_ = DFL->AddCpptrajFile(analyzeArgs.GetStringKey("outthermo"), "'thermo' output",
                                       DataFileList::TEXT, true);
  ...
  modes_ = (DataSet_Modes*)DSL->AddSet( DataSet::MODES, md, "Modes" );
