                                                     ...
                                                     PairwiseMatrix const& pmatrix)
{
  ...
  if ( useGaussianKernel_ )
    err = Cluster_GaussianKernel(framesToCluster, pmatrix);
  else
    err = Cluster_DiscreteDensity(framesToCluster, pmatrix);
  if (err != 0) return 1;
      ...
      for (unsigned int i0 = *idx0; i0 != *(idx0+1); ++i0)
      {
                                    ...
                                    idx1 != C_start_stop.end(); idx1 += 2)
        {
          ...
          for (unsigned int i1 = *idx1; i1 != *(idx1+1); i1++)
          {
            ...
            if (pmatrix.Frame_Distance(point.Fnum(), other_point.Fnum()) < epsilon_) {
                                                               ...
                                                               PairwiseMatrix const& pmatrix)
{
  ...
  Points_.clear();
  int oidx = 0;
  for (Cframes::const_iterator frame = framesToCluster.begin();
                               frame != framesToCluster.end(); ++frame)
  ...
  unsigned int n_distances = (framesToCluster.size() * (framesToCluster.size()-1)) / 2;
  Distances.reserve( n_distances );
  for (Cframes::const_iterator f1 = framesToCluster.begin(); f1 != framesToCluster.end(); ++f1)
    for (Cframes::const_iterator f2 = f1 + 1; f2 != framesToCluster.end(); ++f2)
      Distances.push_back( pmatrix.Frame_Distance( *f1, *f2 ) );
  ...
  for (unsigned int i = 0; i != Points_.size(); i++) {
    for (unsigned int j = i+1; j != Points_.size(); j++) {
      double dist = pmatrix.Frame_Distance(Points_[i].Fnum(), Points_[j].Fnum());
  ...
  for (unsigned int ii = 1; ii != Points_.size(); ii++) {
    ...
    for (unsigned int jj = 0; jj != ii; jj++) {
      ...
      double dist = pmatrix.Frame_Distance(Points_[ord_i].Fnum(), Points_[ord_j].Fnum());
                                                                ...
                                                                PairwiseMatrix const& pmatrix)
{
                        ...
                        point0 != Points_.end(); ++point0)
  {
    ...
    {
      if (point0 != point1) {
        double dist = pmatrix.Frame_Distance(point0->Fnum(), point1->Fnum());
        maxDist = std::max(maxDist, dist);
        if ( dist < epsilon_ )
          density++;
      }
  ...
  for (unsigned int idx0 = 0; idx0 != lastidx; idx0++)
  {
    ...
    for (unsigned int idx1 = idx0+1; idx1 != Points_.size(); idx1++)
    {
      ...
      double dist1_2 = pmatrix.Frame_Distance(point0.Fnum(), point1.Fnum());
...
int Cpptraj::Cluster::Algorithm_DPeaks::ChoosePointsAutomatically() {
...
/** This should never be called for the point with highest density
  ...
  */
void Cpptraj::Cluster::Algorithm_DPeaks::AssignClusterNum(int idx, int& cnum) {
  ...
  if (cnum == -1) {
    mprinterr("Internal Error: In Algorithm_DPeaks::AssignClusterNum could not get"
